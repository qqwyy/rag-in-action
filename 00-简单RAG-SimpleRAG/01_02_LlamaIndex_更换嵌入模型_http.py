...
# 最新模型：bge-large-zh-v1.5   bge-small-zh
os.environ["TOKENIZERS_PARALLELISM"] = "false"  # 禁用并行 tokenization
embed_model = HuggingFaceEmbedding(
    model_name="BAAI/bge-small-zh-v1.5" # 模型路径和名称（首次执行时会从HuggingFace下载  到本地 ～/.cache/huggingface/hub/models--BAAI--bge-m3 ）
)

# 加载数据
documents = SimpleDirectoryReader(input_files=["90-文档-Data/黑悟空/设定.txt"]).load_data()
print(f"xxxxx{len(documents)}")
